<template>
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="grid grid-cols-1">
            <button @click="formToMakeQuestionsOpen"
    ...
    <div class="relative overflow-x-auto">
        <table class="w-full table-auto text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">
                        <div class="flex items-center">
                            <div class="flex flex-col mr-1">
                                <button @click="sortByHeader('tingkat', 'asc')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m4.5 15.75 7.5-7.5 7.5 7.5" />
                                ...
                                <button @click="sortByHeader('tingkat', 'desc')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                    ...
                    <th scope="col" class="px-6 py-3">
                        <div class="flex items-center">
                            <div class="flex flex-col mr-1">
                                <button @click="sortByHeader('jurusan', 'asc')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m4.5 15.75 7.5-7.5 7.5 7.5" />
                                ...
                                <button @click="sortByHeader('jurusan', 'desc')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m19.5 8.25-7.5 7.5-7.5-7.5" />
            ...
            <tbody>
                <template v-if="questionsDataTemp.length == 0">
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th class="text-xl text-center py-2.5" colspan="9">
                            Data Soal Kosong
                        </th>
                    </tr>
                </template>
                <template v-for="(item, index) in questionsDataTemp" :key="index">
                    <tr v-show="checkView(index + 1)"
                        ...
                        <td class="px-4 py-2 text-xs text-center">
                            <template v-if="item.jurusan == null">
                                Semua Jurusan
                            </template>
                            <template v-else>
                                {{ item.jurusan }}
                        ...
                        <td class="grid grid-cols-4 px-4 py-2 gap-1">
                            <button class="bg-sky-200 p-1 text-gray-900 rounded-md inline-flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-4 -h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <RouterLink
                                :to="{ name: 'Question Management Item Admin Page', params: { idSoal: encryption(item.id) } }"
                                target="_blank"
                                class="bg-yellow-200 p-1 text-gray-900 rounded-md inline-flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <button @click="formToEditQuestionOpen(item.id)"
                                class="bg-blue-200 p-1 text-gray-900 rounded-md inline-flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <button @click="dialogToDeleteQuestionOpen(item.id)"
                                class="p-1 text-red-500 rounded-md inline-flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    methods: {
        loadQuestionsData() {
            axios.get(`${this.IP_ENDPOINT}/admin/soal`, {
                ...
                .then(({ data }) => {
                    // console.log(data);
                    this.questionsDataTemp = this.questionsData = data.data;
                    this.pagination.lastPage = Math.ceil(data.data.length / this.view);
                    this.pagination.total = data.data.length;

                    // this.view = data.data.length;

                    this.initView();

                    this.$emit("questionsData", data.data);
        ...
        listViewBinding() {
            ...
            for (let index = 0; index < this.listView.length; index++) {
                if (this.listView[index] < this.questionsDataTemp.length) {
        ...
        changePage(page) {
            if (page >= 1 && page <= this.pagination.lastPage) {
                ...
                const total = this.questionsDataTemp.length;
        ...
        search(value) {
            if (value.length >= 1) {
                ...
                console.log(this.questionsDataTemp);
            } else {
                this.questionsDataTemp = this.questionsData;
            }
        },
        sortByHeader(key, order) {
            this.questionsDataTemp.sort((a, b) => {
                let valA = a[key];
                let valB = b[key];
                ...
                // Convert to string if not numbers, and perform case-insensitive comparison
        ...
        dialogToDeleteQuestionOpen(id) {
            ...
            this.$emit('isDialogToDeleteQuestionTrue', data);
        }
    }
}
</script>
<template>
    <BaseLayout>
        <template #content>
            <div class="py-5">
                <h1 class="text-4xl font-sans font-semibold">Data Soal</h1>
            </div>
            ...
            <div class="relative overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <template v-if="dataSoalTemp.length == 0">
                            <tr
                        ...
                        <template v-for="(item, index) in dataSoalTemp" :key="index">
                            <tr v-show="checkView(index + 1)"
                                ...
                                <td class="px-6 py-4">
                                    <template v-if="item.tipeSoal == 'PILIHAN_GANDA'">
                                        {{ item.namaSoal + " (PG)" }}
                                    </template>
                                    <template v-else>
                                        {{ item.namaSoal + " (E)" }}
                                ...
                                <td class="px-6 py-4 space-x-2 text-right">
                                    ...
                                    <button class="px-2.5 py-1 text-red-500 rounded-md inline-flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    methods: {
        ...
        loadDataSoal() {
            axios.get(`${this.IP_ENDPOINT}/admin/soal`, {
                ...
                .then(({ data }) => {
                    // console.log(data);
                    this.dataSoalTemp = this.dataSoal = data.data;
        ...
        listViewBinding() {
            ...
            for (let index = 0; index < this.listView.length; index++) {
                if (this.listView[index] < this.dataSoalTemp.length) {
                    list.push(this.listView[index]);
                }
            }

            const itemsLength = parseInt(JSON.stringify(this.dataSoalTemp.length));
        ...
        changePage(page) {
            if (page >= 1 && page <= this.pagination.lastPage) {
                ...
                const total = this.dataSoalTemp.length;
        ...
        search(value) {
            if (value.length >= 1) {
                const options = {
                    shouldSort: true,
                    keys: ['namaSoal', 'tingkat', 'jurusan'],
                    threshold: 0
                };
                const fuse = new Fuse(this.dataSoal, options);
                this.dataSoalTemp = fuse.search(value).map(elem => elem.item);
            } else {
                this.dataSoalTemp = this.dataSoal;
            }
        },
        sortByHeader(key, order) {
            this.dataSoalTemp.sort((a, b) => {
                let valA = a[key];
                let valB = b[key];
